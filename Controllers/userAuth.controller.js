...
// Utility function to run SQL queries with promises
...
const SignUp = asyncHandler(async (req, res) => {
    try {
        ...
        const checkQuery = "SELECT * FROM users WHERE phoneNumber = ?";
        const checkResult = await runQuery(checkQuery, [formattedPhoneNumber]);
        ...
        const insertQuery = "INSERT INTO users(`firstName`, `lastName`, `phoneNumber`, `NICnumber`) VALUES(?, ?, ?, ?)";
        await runQuery(insertQuery, [firstName, lastName, formattedPhoneNumber, NICnumber]);
...
const loginController = asyncHandler(async (req, res) => {
    ...
    try {
        const sql = "SELECT * FROM users WHERE phoneNumber = ? LIMIT 1";
        const results = await runQuery(sql, [phoneNumber]);
...
const getProfileDetails = asyncHandler(async (req, res) => {
    ...
    try {
        const sql = "SELECT firstName, lastName, phoneNumber, NICnumber FROM users WHERE id = ?";
...
const updatePhoneNumber = asyncHandler(async (req, res) => {
    ...
    try {
        const sql = "UPDATE users SET phoneNumber = ? WHERE id = ?";
        const results = await runQuery(sql, [newPhoneNumber, userId]);
...
// Other controllers can be similarly refactored...

...
exports.loginUserSchema = Joi.object({
  phonenumber: Joi.string().pattern(/^[0-9+]{10,15}$/).required().label('Phone number')
});
...
exports.updatePhoneNumberSchema = Joi.object({
  newPhoneNumber: Joi.string().pattern(/^[0-9+]{10,15}$/).required().label('New Phone Number')
});

exports.signupCheckerSchema = Joi.object({
  phoneNumber: Joi.string().pattern(/^[0-9+]{10,15}$/).optional().label('Phone Number'),
  NICnumber: Joi.string().min(9).max(12).optional().label('NIC Number')
